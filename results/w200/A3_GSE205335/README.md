# A3 — GSE205335 malignant TACSTD2 versus T/NK

## Result

The requested contrast is feasible from open processed GEO data. TACSTD2 was
strongly enriched in author-labelled malignant cells relative to T/NK cells in
every one of 22 evaluable patients:
...
The secondary ICI-response analysis was null: six responders versus ten
non-responders had p = 0.96 for mean normalized expression, p = 0.71 for
percent positive, and p = 0.64 for pseudobulk CPM. These small, heterogeneous
groups cannot exclude a moderate response association.
...
(three responders and one non-responder), so the response comparison is
...
- `stats_results.txt`: readable statistical report
...
- `tacstd2_summary.png`: response and paired-compartment panels
