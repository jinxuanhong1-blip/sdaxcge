#!/usr/bin/env python3
"""CLAIM A1 (LUSC-only): TACSTD2 (TROP2) expression vs immune features in
TCGA-LUSC primary tumors, adjusting for ABSOLUTE tumor purity.

LUSC only. Not LUAD. Not pooled NSCLC.

Inputs (downloaded, see provenance.json for URLs/md5):
  data/TCGA.LUSC.HiSeqV2.gz                        Xena TCGA-LUSC RNA-seq, log2(RSEM norm_count + 1)
...
OUT = os.path.join(ROOT, "results", "w200", "A1_LUSC")
...
INPUTS = {
    "expression": {
        ...
        "desc": "UCSC Xena TCGA-LUSC gene expression, log2(RSEM normalized_count + 1)",
    },
    "purity": {
...
def partial_spearman(x, y, z):
    """Partial Spearman of x,y controlling for z.

    Pearson on ranks of residuals after regressing ranked z out of ranked x and y.
    t-test uses n-3 df (one covariate). Same estimator as the LUAD-only A1 script.
    """
    xr = stats.rankdata(x)
    yr = stats.rankdata(y)
    zr = stats.rankdata(z)
...
expr = pd.read_csv(os.path.join(DATA, "TCGA.LUSC.HiSeqV2.gz"), sep="\t", index_col=0)
...
sig = sig[sig.index != ""]
sig = sig.loc[sig.index.isin(eg.index)]  # restrict pan-cancer table to LUSC samples
...
res.to_csv(os.path.join(OUT, "a1_lusc_correlations.tsv"), sep="\t", index=False)
df.reset_index().to_csv(os.path.join(OUT, "a1_lusc_analysis_table.tsv"), sep="\t", index=False)
...
fig.suptitle("TCGA-LUSC primary tumors: TACSTD2 vs immune features", fontsize=12)
...
ax.set_title("TCGA-LUSC: purity-adjusted association of TACSTD2 with immune features\n"
...
# ---------------- provenance ----------------
prov = {
    "claim": "A1 (LUSC-only): TACSTD2 expression is associated with immune features in "
             "TCGA-LUSC primary tumors after adjusting for tumor purity",
    "cohort": "TCGA-LUSC primary tumor samples (-01) with RNA-seq and ABSOLUTE purity",
    "histology": "LUSC only (not LUAD, not pooled NSCLC)",
