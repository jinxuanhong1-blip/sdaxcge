#!/usr/bin/env python3
"""Download public DepMap 24Q4 and Gygi CCLE files for the TROP2 partial test.

Keeps only the genes this analysis scores, then deletes the full matrices.
...
UA = "sdaxcge-depmap-trop2-ifn-partial/1.0 (+https://github.com/jinxuanhong1-blip/sdaxcge)"

MODEL_URL = "https://ndownloader.figshare.com/files/51065297"
EXPR_URL = "https://ndownloader.figshare.com/files/51065489"
CRISPR_EFFECT_URL = "https://ndownloader.figshare.com/files/51064667"
CRISPR_DEP_URL = "https://ndownloader.figshare.com/files/51064631"
GMT_URL = (
...
# Pre-specified. Hallmark IFN-γ members are added from the GMT at runtime.
CORE_GENES = [
    "TACSTD2",
    "CLDN4",
    "CD274",
...
CRISPR_GENES = ["TACSTD2", "CLDN4", "KRAS", "EEF2"]
...
def extract_model_by_gene(src: Path, wanted: set[str], dest: Path) -> dict:
    """Stream a models × genes CSV and keep the first column plus wanted symbols."""
    ...
    return {
        ...
        "source_header_gene_example": symbol_of(header[1]) if len(header) > 1 else None,
    }


def extract_gygi(src: Path, wanted: set[str], dest: Path) -> dict:
    """Stream Gygi genes × samples and keep wanted gene symbols."""
    ...
    with opener(src, "rt", newline="") as fh:
        ...
        with dest.open("w", newline="") as out:
            writer = csv.writer(out)
            writer.writerow(
                ["Gene_Symbol", "Protein_Id"] + [header[i] for i in quant_idx]
            )
            for row in reader:
                if sym_i >= len(row):
                    continue
...
def main() -> int:
    ...
    ap.add_argument("--outdir", default="data/depmap_trop2_ifn_partial_cldn4")
    args = ap.parse_args()
    ...
    if not expr_slim.exists() or expr_slim.stat().st_size < 50_000:
        ...
        meta = {
            ...
            "note": "slim extract already present; full-file sha256 not recomputed",
        }
        print(f"OK exists {expr_slim}", flush=True)
    manifest["files"]["expression"] = {"url": url, "sha256": digest, "extract": meta}

    for label, url0, fname in (
        ("crispr_gene_effect", CRISPR_EFFECT_URL, "CRISPRGeneEffect.csv"),
        ("crispr_gene_dependency", CRISPR_DEP_URL, "CRISPRGeneDependency.csv"),
    ):
        full = outdir / fname
        slim = outdir / fname.replace(".csv", "_panel.csv")
        if not slim.exists() or slim.stat().st_size < 1_000:
            if not full.exists() or full.stat().st_size < 100_000_000:
                url, digest = fetch([url0], full, 100_000_000)
            else:
                url, digest = url0, sha256_file(full)
            meta = extract_model_by_gene(full, set(CRISPR_GENES), slim)
            full.unlink(missing_ok=True)
        else:
            url, digest = url0, None
            with slim.open() as fh:
                header = next(csv.reader(fh))
            meta = {
                "genes_kept": header[1:],
                "missing_genes": sorted(set(CRISPR_GENES) - set(header[1:])),
                "note": "slim extract already present",
            }
            print(f"OK exists {slim}", flush=True)
        manifest["files"][label] = {"url": url, "sha256": digest, "extract": meta}
    ...
    if not gygi_slim.exists() or gygi_slim.stat().st_size < 10_000:
        ...
        meta = extract_gygi(gygi, wanted_expr | {"TACSTD2", "CLDN4", "CD274"}, gygi_slim)
