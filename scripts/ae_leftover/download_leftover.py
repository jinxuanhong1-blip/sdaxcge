#!/usr/bin/env python3
"""Download leftover processed files that are strictly under 2 GB."""
...
LIMIT = 2_000_000_000
DOWNLOAD_DECISIONS = {"leftover_analyze"}
METADATA_DECISIONS = DOWNLOAD_DECISIONS | {"leftover_metadata", "leftover_duplicate"}
...
def download(url: str, destination: Path, expected_size: int) -> None:
    ...
    for attempt in range(4):
        try:
            request = urllib.request.Request(url, headers={"User-Agent": "ae-leftover/1.0"})
            with urllib.request.urlopen(request, timeout=180) as response, partial.open("wb") as output:
                while chunk := response.read(4 * 1024 * 1024):
                    output.write(chunk)
            if expected_size and partial.stat().st_size != expected_size:
                raise IOError(
                    f"size mismatch for {destination}: {partial.stat().st_size} != {expected_size}"
                )
...
def main() -> None:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path("results/w200/AE_leftover/downloads"),
    )
    parser.add_argument(
        "--download-manifest",
        type=Path,
        default=Path("results/w200/AE_leftover/download_manifest.json"),
    )
    ...
    for study in search["studies"]:
        ...
        eligible = []
        if study["decision"] in DOWNLOAD_DECISIONS:
            for item in study.get("processed_files", []):
                eligible.append({**item, "role": "processed"})
        for item in study.get("metadata_files", []):
            eligible.append({**item, "role": "metadata"})
        for item in eligible:
            if item["role"] == "processed" and not (0 < item["size"] < LIMIT):
            ...
            print(f"{accession}: {item['path']} ({item['size']} bytes)", flush=True)
