"""Effect-size helpers for the TCGA TACSTD2 versus CD8/CD3 keratin sweep.
...
def select_abs(effects, neg_counts, cohorts, pvals):
    """Index of the eligible row with the largest |effect|.

    Eligible: finite negative effect, at least 6 cohorts negative, and at
    least 6 cohorts in the pool. Untested cohorts are not counted as negative.
    Tie-break: more negative cohorts, then more cohorts in the pool, then
    smaller p, then the earlier row.
    Returns -1 when nothing is eligible.
    """
    effects = np.asarray(effects, dtype=float)
    neg = np.asarray(neg_counts, dtype=float)
    cohorts_n = np.asarray(cohorts, dtype=float)
    pvals = np.asarray(pvals, dtype=float)
    eligible = np.isfinite(effects) & (effects < 0) & (neg >= 6) & (cohorts_n >= 6)
    if not np.any(eligible):
        return -1
    idx = np.flatnonzero(eligible)
    abs_e = np.abs(effects[idx])
    score_p = np.where(np.isfinite(pvals[idx]), pvals[idx], np.inf)
    order = np.lexsort((idx, score_p, -cohorts_n[idx], -neg[idx], -abs_e))
    return int(idx[order[0]])


def select_positive(effects, cohorts, pvals):
    """Index of the largest positive pooled effect with at least 6 cohorts.

    This is the opposite-direction extreme. It is reported so the negative
    maximum is not the only extreme in the grid. Returns -1 when none qualify.
    """
    effects = np.asarray(effects, dtype=float)
    cohorts_n = np.asarray(cohorts, dtype=float)
    pvals = np.asarray(pvals, dtype=float)
    ok = np.isfinite(effects) & (effects > 0) & (cohorts_n >= 6)
    if not np.any(ok):
        return -1
    idx = np.flatnonzero(ok)
    score_p = np.where(np.isfinite(pvals[idx]), pvals[idx], np.inf)
    order = np.lexsort((idx, score_p, -effects[idx]))
    return int(idx[order[0]])
