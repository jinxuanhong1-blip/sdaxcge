...
Used by the GEO 2010-2014 leftover human lung ICI / PD-1 / PD-L1 mining
pipeline. All network access is read-only against public NCBI endpoints.
Nothing here invents or hard-codes GSE/GSM/GPL IDs.
"""
import json
import time
import urllib.parse
...
def _get_bytes(url, tries=5, timeout=300):
    last_err = None
    for i in range(tries):
        _throttle()
        try:
...
def esearch(db, term, retmax=1000):
    """Return list of UIDs for a term (real NCBI esearch)."""
    ids = []
    retstart = 0
    while True:
        params = {
