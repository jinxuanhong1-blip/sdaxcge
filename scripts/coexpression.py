...
This is the "B1 analog" for TCGA-BRCA: it reproduces, for the breast-cancer
...
* Samples: TCGA-BRCA primary tumours (barcode sample-type code ``01``) by
...
DATA_DIR = ROOT / "data"
DEFAULT_EXPR = DATA_DIR / "TCGA-BRCA.HiSeqV2.gz"
...
def make_plots(
    ...
    if focus in expr.index:
        ...
        ax.set_title(
            f"{anchor} vs {focus} (TCGA-BRCA, n={len(samples)})\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--outdir", type=Path, default=ROOT / "results" / "w200" / "B1_BRCA")
    ...
    if q.get("in_universe"):
        ...
        lines = [
            f"# {args.focus} vs {args.anchor} co-expression in TCGA-BRCA (honest ranking)",
            "",
            ...
            f"**Answer:** {verdict}",
            "",
            f"- Cohort: TCGA-BRCA, {len(samples)} primary-tumour samples "
