...
This is the "B1 analog" for TCGA-STAD: it reproduces, for the stomach
...
* Samples: TCGA-STAD primary tumours (barcode sample-type code ``01``) by
...
DATA_DIR = ROOT / "data"
DEFAULT_EXPR = DATA_DIR / "TCGA-STAD.HiSeqV2.gz"
...
def compute_ranking(
    ...
    # Zero-variance genes yield NaN correlations and cannot be ranked.
    # They stay in the table with missing ranks so the drop is visible.
    df = df.sort_values("spearman_r", ascending=False, kind="mergesort", na_position="last").reset_index(drop=True)
    finite_s = df["spearman_r"].notna()
    ranks_s = np.full(len(df), np.nan)
    ranks_s[finite_s.to_numpy()] = np.arange(1, int(finite_s.sum()) + 1)
    df.insert(1, "spearman_rank", ranks_s)
    df["pearson_rank"] = df["pearson_r"].rank(ascending=False, method="min")
    return df
...
def focus_report(df: pd.DataFrame, focus: str, n_universe: int) -> dict:
    ...
    r = row.iloc[0]
    s_rank = None if pd.isna(r["spearman_rank"]) else int(r["spearman_rank"])
    p_rank = None if pd.isna(r["pearson_rank"]) else int(r["pearson_rank"])
    n_ranked = int(df["spearman_r"].notna().sum())
    return {
        "gene": focus,
        "in_universe": True,
        ...
        "spearman_r": None if pd.isna(r["spearman_r"]) else float(r["spearman_r"]),
        "spearman_q": None if pd.isna(r["spearman_q"]) else float(r["spearman_q"]),
        "spearman_percentile": (
            None if s_rank is None else round(100 * (1 - (s_rank - 1) / n_ranked), 3)
        ),
        "pearson_rank": p_rank,
        "pearson_r": None if pd.isna(r["pearson_r"]) else float(r["pearson_r"]),
        "pearson_q": None if pd.isna(r["pearson_q"]) else float(r["pearson_q"]),
        "pearson_percentile": (
            None if p_rank is None else round(100 * (1 - (p_rank - 1) / n_ranked), 3)
        ),
        "n_ranked_surface_genes": n_ranked,
        "n_unrankable_zero_variance": int(len(df) - n_ranked),
    }
...
def make_plots(
    ...
    if focus in expr.index:
        ...
        ax.set_title(
            f"{anchor} vs {focus} ({COHORT}, n={len(samples)})\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--outdir", type=Path, default=ROOT / "results" / "w200" / "B1_STAD")
    ...
    summary = {
        ...
        "cohort": COHORT,
        "expression_dataset": "UCSC Xena TCGA.STAD.sampleMap/HiSeqV2 (log2 norm_count+1)",
        ...
        "n_ranked_surface_genes": report.get("n_ranked_surface_genes", n_universe),
        "n_unrankable_zero_variance": report.get("n_unrankable_zero_variance", 0),
        "unrankable_genes": df.loc[df["spearman_r"].isna(), "gene"].tolist(),
        ...
        "top10_spearman": [
            {
                "gene": rec["gene"],
                "spearman_rank": int(rec["spearman_rank"]),
                "spearman_r": float(rec["spearman_r"]),
                "spearman_q": float(rec["spearman_q"]),
            }
            for rec in df.head(10)[["gene", "spearman_rank", "spearman_r", "spearman_q"]].to_dict("records")
        ],
    }
    ...
    if q.get("in_universe"):
        ...
        n_ranked = q.get("n_ranked_surface_genes", n_universe)
        n_drop = q.get("n_unrankable_zero_variance", 0)
        drop_note = (
            f" {n_drop} surfaceome gene(s) had zero variance and were left unranked."
            if n_drop
            else ""
        )
        lines = [
            ...
            f"matrix (anchor removed).{drop_note}",
            f"- Ranked genes (finite Spearman ρ): {n_ranked}.",
