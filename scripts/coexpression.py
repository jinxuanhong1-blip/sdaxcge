...
This is the "B1 analog" for TCGA-READ: it reproduces, for the rectal
...
* Samples: TCGA-READ primary tumours (barcode sample-type code ``01``) by
...
DATA_DIR = ROOT / "data"
DEFAULT_EXPR = DATA_DIR / "TCGA-READ.HiSeqV2.gz"
DEFAULT_SURF = DATA_DIR / "table_S3_surfaceome.xlsx"
COHORT = "TCGA-READ"
EXPR_LABEL = "log2(norm_count+1)"
...
def make_plots(
    ...
    if focus in expr.index:
        ...
        ax.set_xlabel(f"{anchor} {EXPR_LABEL}")
        ax.set_ylabel(f"{focus} {EXPR_LABEL}")
        ax.set_title(
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--outdir", type=Path, default=ROOT / "results" / "w200" / "B1_READ")
    ...
    summary = {
        ...
        "cohort": COHORT,
        "expression_dataset": "UCSC Xena TCGA.READ.sampleMap/HiSeqV2 (log2 norm_count+1)",
