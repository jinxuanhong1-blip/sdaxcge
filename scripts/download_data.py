...
1. TCGA-PRAD RNA-seq gene-expression matrix from the UCSC Xena TCGA hub
   (dataset ``TCGA.PRAD.sampleMap/HiSeqV2``). Values are log2(norm_count + 1),
   ...
   focus genes (TACSTD2, CLDN4) directly addressable. This is the same matrix
   family used by the B1 BRCA analog (``results/w200/B1_BRCA``).

2. The "in silico human surfaceome" master table (Bausch-Fluck et al.,
   PNAS 2018). The canonical host (wlab.ethz.ch / wollscheidlab.org) now
   serves a single-page app or Git LFS pointers for the file path, so we
   pull the identical workbook that is vendored in the
   steveneschrich/surfaceome R package on GitHub.
...
EXPRESSION_URL = (
    ...
    "download/TCGA.PRAD.sampleMap%2FHiSeqV2.gz"
)
EXPRESSION_FILE = DATA_DIR / "TCGA-PRAD.HiSeqV2.gz"
