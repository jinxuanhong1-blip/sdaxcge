...
def write_finding(outdir: Path, summary: dict) -> None:
    ...
    lines = [
        ...
        f"- Cluster-malignant ineligible post samples (n_malignant < {MIN_MAL}): {primary.get('dropped') or 'none'}.",
        f"- Marker-malignant ineligible post samples: {marker.get('dropped') or 'none'}.",
        f"- A3-malignant ineligible post samples: {a3.get('dropped') or 'none'}.",
        "",
        "## Integration",
        "",
        f"- Method: **{method}**",
