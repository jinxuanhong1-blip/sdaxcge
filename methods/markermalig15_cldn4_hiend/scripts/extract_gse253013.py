...
# Malignant / epithelial / T / NK / QC panel (A3-style, plus CLDN4).
GENE_PANEL = [
    "TACSTD2",
    "CLDN4",
    "EPCAM",
...
def load_panel(path: Path | None) -> list[str]:
    if path is None or not path.exists():
        return list(GENE_PANEL)
    genes = [ln.strip() for ln in path.read_text().splitlines() if ln.strip() and not ln.startswith("#")]
    # keep lineage defaults first so the gene-name vector is still identifiable
    seen = set()
    out = []
    for g in list(GENE_PANEL) + genes:
        if g not in seen:
            seen.add(g)
            out.append(g)
    return out
...
def main() -> int:
    ...
    ap.add_argument("--outdir", type=Path, default=Path("data/gse253013/extracted"))
    ap.add_argument("--panel", type=Path, default=None, help="extra gene symbols, one per line")
    args = ap.parse_args()
    ...
    gene_vec = find_gene_vector(ex.str_vecs, panel)
    gene_present = []
    gene_idx = {}
        ...
        json.dumps(
            {
                ...
                "absent": [g for g in panel if g not in gene_idx],
                "index": gene_idx,
            },
            indent=2,
        )
