#!/usr/bin/env python3
"""Build a joint malignant/epithelial AnnData for GSE131907 + GSE205335.

ADDITIVE. Malignant/epithelial only. GSE207422 and GSE148071 are not added.
GSE131907: author Epithelial cells on tLung / nLung / tL/B / mLN / mBrain
(PE unlabeled epithelium is dropped). nLung AT2 is kept for the biological root.
...
def main() -> None:
    ...
    p.add_argument("--data", type=Path, default=Path("/tmp/winpair_slingshot_real"))
    p.add_argument(
        "--out", type=Path, default=Path("/tmp/winpair_slingshot_real/epithelium.h5ad")
    )
