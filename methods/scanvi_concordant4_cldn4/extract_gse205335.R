...
take <- function(x, k) {
  if (!length(x) || k <= 0) return(character())
...
for (pat in keep_pats) {
  ...
  mal <- tumor_qc$mat_barcode[tumor_qc$author_malignant]
  tnk <- tumor_qc$mat_barcode[tumor_qc$author_tnk & !tumor_qc$author_malignant]
  oth <- setdiff(tumor_qc$mat_barcode, c(mal, tnk))
  chosen <- c(take(mal, MAL_CAP), take(tnk, TNK_CAP), take(oth, OTHER_CAP))
  if (!length(chosen)) next
  chosen <- tumor_qc[match(chosen, tumor_qc$mat_barcode), , drop = FALSE]
  ...
  say("  ", pat, "cells", n_cells, "mal", n_mal, "tnk", n_tnk, "sub", nrow(chosen), "pct", round(unit_rows[[pat]]$mal_CLDN4_pct, 2))
}
