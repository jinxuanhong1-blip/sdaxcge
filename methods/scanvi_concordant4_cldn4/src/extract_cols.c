...
int main(int argc, char **argv) {
    ...
    while ((header = reader_next(&rd, &llen)) != NULL) {
        ...
        while (lp < le && col < ncol) {
            int need = sub_of[col] >= 0 || (is_cldn4 && want_cld[col]);
            float v = 0.f;
            ...
            if (is_cldn4 && next_cld < n_cld && cld_cols[next_cld] == col) {
                cld_vals[next_cld] = v;
                next_cld++;
            } else if (is_cldn4 && want_cld[col]) {
                /* columns are in order; if we skipped because next doesn't match, scan */
            }
            col++;
            if (lp < le && *lp == '\t') lp++;
        }
        if (is_cldn4) {
            /* The walk above only fills cld_vals when next_cld tracks columns in order.
               Re-parse CLDN4 properly if the tracker missed (it should not). */
            cldn4_done = 1;
    ...
    fprintf(mf, "n_genes\t%d\nn_cells\t%d\nnnz\t%ld\ncldn4_found\t%d\n", gene_i, n_sub, nnz,
            cldn4_done);
    fclose(mf);
    fprintf(stderr, "done genes=%d cells=%d nnz=%ld cldn4=%d\n", gene_i, n_sub, nnz, cldn4_done);
