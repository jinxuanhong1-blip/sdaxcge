#!/usr/bin/env python3
"""Build full-unit CLDN4/TACSTD2/T/NK tables and a 10% h5ad per GSE.

The patient / donor / sample table uses every malignant cell in the locked unit.
The h5ad keeps each QC-pass cell with probability 0.10 (stable_keep), so
embedded T/NK counts stay proportional to the unit. % positive is not taken
from that sample.
"""
...
from stable_keep import stable_keep
...
def unit_row(dataset, unit_id, unit_type, tissue, n_cells, n_mal, n_tnk, umi, tac_umi, malig_def) -> dict:
    pct, mean = cldn4_stats(umi)
    tpct, tmean = cldn4_stats(tac_umi)
    return {
        "dataset": dataset,
        "unit_id": unit_id,
        "patient_id": unit_id,
        ...
        "mal_TACSTD2_pct": tpct,
        "mal_TACSTD2_mean_log1p": tmean,
        "malig_def": malig_def,
    }
...
def write_h5ad(path: Path, mat_gxc: sparse.csr_matrix, genes: np.ndarray, meta: pd.DataFrame) -> None:
    ...
    # Drop all-zero genes to keep the subsample small. Both tested genes stay.
    keep = np.asarray(mat_gxc.sum(axis=1)).ravel() > 0
    for forced in ("CLDN4", "TACSTD2"):
        if forced in set(genes):
            ix = int(np.where(genes == forced)[0][0])
            keep[ix] = True
...
def prepare_gse123902(raw: Path) -> None:
    ...
    for rec in locked.itertuples(index=False):
        ...
        if "CLDN4" not in gindex or "TACSTD2" not in gindex:
            raise SystemExit(f"CLDN4 or TACSTD2 missing in {path.name}")
        mal, tnk = marker_masks(mat, gindex)
        cld = gene_vec(mat, gindex, "CLDN4")
        tac = gene_vec(mat, gindex, "TACSTD2")
        units.append(
            unit_row(
                "GSE123902", rec.patient, "donor", rec.tissue, mat.shape[1],
                int(mal.sum()), int(tnk.sum()), cld[mal], tac[mal], "marker_malig",
            )
        )
        qc = qc_mask(mat, genes)
        barcodes_all = df.index.to_numpy().astype(str)
        cols = np.flatnonzero([
            bool(qc[i]) and stable_keep(f"{rec.patient}|{barcodes_all[i]}")
            for i in range(mat.shape[1])
        ])
        if len(cols) == 0:
            print(f"  no QC cells sampled for {rec.patient}", flush=True)
            continue
        sub = mat[:, cols]
...
def prepare_gse189357(raw: Path) -> None:
    ...
    for pat in patients:
        ...
        gindex = gene_index(genes)
        if "CLDN4" not in gindex or "TACSTD2" not in gindex:
            raise SystemExit(f"CLDN4 or TACSTD2 missing in {pat}")
        mal, tnk = marker_masks(mat, gindex)
        cld = gene_vec(mat, gindex, "CLDN4")
        tac = gene_vec(mat, gindex, "TACSTD2")
        units.append(unit_row(
            "GSE189357", pat, "patient", "TUMOR", mat.shape[1],
            int(mal.sum()), int(tnk.sum()), cld[mal], tac[mal], "marker_malig",
        ))
        qc = qc_mask(mat, genes)
        cols = np.flatnonzero([
            bool(qc[i]) and stable_keep(f"{pat}|{barcodes[i]}")
            for i in range(mat.shape[1])
        ])
        if len(cols) == 0:
            print(f"  no QC cells sampled for {pat}", flush=True)
            continue
        sub = mat[:, cols]
        pieces.append((sub, genes))
...
def prepare_gse131907(raw: Path) -> None:
    ...
    with flags.open("w") as fh:
        seen = set()
        for sample, idx, is_mal in zip(ann["Sample"], ann["Index"], ann["author_malignant"]):
            if idx in seen:
                continue
            seen.add(idx)
            sub = 1 if stable_keep(f"{sample}|{idx}") else 0
            cld = 1 if idx in mal_ids else 0
            if sub or cld:
                fh.write(f"{idx} {sub} {cld}\n")
            n_sub_flag += sub
    print(f"GSE131907 flagged subsample cells before QC: {n_sub_flag}", flush=True)
    ...
    if int(meta_kv["cldn4_found"]) != 1 or int(meta_kv.get("tacstd2_found", 0)) != 1:
        raise SystemExit(f"CLDN4/TACSTD2 row missing in GSE131907: {meta_kv}")
    ...
    tac = pd.read_csv(str(prefix) + ".tacstd2.tsv", sep="\t", header=None, names=["Index", "tac"])
    cld = cld.drop_duplicates("Index")
    tac = tac.drop_duplicates("Index")
    ann_m = ann.loc[ann["author_malignant"], ["Index", "Sample"]].merge(cld, on="Index", how="left").merge(tac, on="Index", how="left")
    missing = int(ann_m["umi"].isna().sum() + ann_m["tac"].isna().sum())
    if missing:
        print(f"warning malignant cells missing from CLDN4/TACSTD2 rows: {missing}", flush=True)
    ann_m["umi"] = ann_m["umi"].fillna(0)
    ann_m["tac"] = ann_m["tac"].fillna(0)
    units = []
    for sample, g in ann.groupby("Sample", sort=True):
        n_cells_s = len(g)
        n_mal = int(g["author_malignant"].sum())
        n_tnk = int(g["author_tnk"].sum())
        sub = ann_m.loc[ann_m["Sample"] == sample]
        origin = g["Sample_Origin"].iloc[0]
        units.append(unit_row(
            "GSE131907", sample, "sample", origin, n_cells_s, n_mal, n_tnk,
            sub["umi"].to_numpy(dtype=float), sub["tac"].to_numpy(dtype=float), "author_malig",
        ))
        print(
            f"  {sample} cells={n_cells_s} mal={n_mal} tnk={n_tnk} "
            f"cldn4={units[-1]['mal_CLDN4_pct']:.2f} tacstd2={units[-1]['mal_TACSTD2_pct']:.2f}",
            flush=True,
        )
    pd.DataFrame(units).to_csv(out / "units.tsv", sep="\t", index=False)
