...
def nhood_membership(nhoods: Nhoods, n_cells: int | None = None) -> list[np.ndarray]:
    """Each neighbourhood as index ∪ k NN. `n_cells` is ignored (legacy arg)."""
    out = []
...
def sample_paired_tnk_by_tacstd2(
    ...
    """Legacy name from the TACSTD2 runner. The extra score vector is unused."""
    return sample_paired_tnk_by_gene(members, sample, is_tnk, high, low)
