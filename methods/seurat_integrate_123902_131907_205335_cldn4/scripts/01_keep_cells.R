#!/usr/bin/env Rscript
# Build capped malignant + T/NK barcode lists.
# Honest unit = patient (GSE123902 donor; GSE131907 sample=patient on this gate;
# GSE205335 patient). GSE123902 keep list is written by 02_extract.py.
suppressPackageStartupMessages({
  library(utils)
})

args <- commandArgs(trailingOnly = TRUE)
geo <- if (length(args) >= 1) args[[1]] else "/tmp/triple_geo"
root <- if (length(args) >= 2) args[[2]] else {
  ca <- commandArgs(trailingOnly = FALSE)
  f <- sub("^--file=", "", ca[grep("^--file=", ca)])
  if (length(f)) dirname(dirname(normalizePath(f))) else getwd()
}
...
MALIG_SUBTYPES <- c("Malignant cells", "tS1", "tS2", "tS3")
...
ann$compartment[ann$Cell_subtype %in% MALIG_SUBTYPES & ann$Sample %in% elig131] <- "malignant"
...
keep131$patient_id <- as.character(keep131$Sample)
