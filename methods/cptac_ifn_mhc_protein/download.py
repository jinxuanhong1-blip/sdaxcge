#!/usr/bin/env python3
"""Download CPTAC LUAD and LSCC public TMT tumor protein plus phenotype (freeze v1.2).

Open S3 only. Filenames from the LinkedOmics CPTAC-pancan index.
Protein matrices are the analysis. Phenotype is used for WES purity
(sensitivity) and to confirm the file has no treatment or ICI column.
No dbGaP / CDS tokens. Files >2 GB are skipped.
"""
...
def main() -> int:
    ...
    p.add_argument("--outdir", default="data/cptac_ifn_mhc_protein")
    args = p.parse_args()
    ...
    manifest = {
        ...
        "scope": (
            "CLDN4 and TACSTD2 protein vs MHC-I / IFN / CD8A protein; "
            "LUAD + LSCC public TMT; phenotype for purity sensitivity only"
        ),
