...
def fetch(url: str) -> bytes:
    req = urllib.request.Request(url, headers={"User-Agent": "sdaxcge-scrna-scenic-cldn4"})
...
def main() -> None:
    ...
    manifest = {
        "purpose": "Public curated TF-target edges for CLDN4-high GRN proxy. Not ChIP.",
        "do_not_invent_chip": True,
        "urls": URLS,
        "n_rows": int(len(df)),
        "focus_tfs": FOCUS,
        "cldn4_listed_as_target": bool(df.target.eq("CLDN4").any()),
        "elf3_cldn4_in_prior": bool(
            ((df.tf == "ELF3") & (df.target == "CLDN4")).any()
        ),
    }
