...
Pair: GSE123902 (marker malignant, donor) + GSE131907 (author malignant, sample).
PR #459 given cut: %pos n=34. Not CellChat. Not T/NK infiltrate. No dual-high.
No GSE148071.
...
def pair_meta() -> pd.DataFrame:
    """PR #459 locked units: GSE123902 n=13 donors + GSE131907 n=21 samples."""
        ...
        rows.append(
            {
                "family": name,
                "split": split,
                "cohort": cohort or "GSE123902+GSE131907",
...
def strip_cldn4(meta: pd.DataFrame, path: Path) -> None:
    ...
    fig.suptitle("PR #459 pair CLDN4-only quartiles (GSE123902 donor / GSE131907 sample)")
    fig.tight_layout()
    fig.savefig(path.with_suffix(".png"), dpi=160)
    fig.savefig(path.with_suffix(".pdf"))
    plt.close(fig)
...
def n_bar(inv: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("Malignant DE sample sizes — GSE123902+GSE131907 only")
...
def write_finding(
    ...
    def score_md(split: str, cohort: str = "GSE123902+GSE131907") -> str:
    ...
    def score_md_cohorts(split: str) -> str:
        sub = fam_score[
            (fam_score["split"] == split) & (fam_score["cohort"] != "GSE123902+GSE131907")
        ]
        if sub.empty:
    ...
    text = f"""# Pair GSE123902+GSE131907 — malignant-cell-intrinsic CLDN4 DE
...
- GSE123902: n={n123} donors (tumor/met; marker-malignant; normals dropped). Q1={int(q123.get('Q1',0))} Q4={int(q123.get('Q4',0))}.
...
def main() -> None:
    ...
    for cohort, path in paths.items():
        if not path.exists():
            raise SystemExit(f"missing {path}; run build_gse123902_pseudobulk.py")
    ...
    if not de_c.empty:
        de_c = de_c.copy()
        de_c["compartment"] = "malignant"
        de_c["cohort"] = "GSE123902+GSE131907"
    ...
    for cohort in (None, "GSE123902", "GSE131907"):
    ...
    if not fam_score.empty:
        ...
        head_fs = fam_score[
            (fam_score["cohort"] == "GSE123902+GSE131907") & (fam_score["split"] == "q4q1")
        ]
