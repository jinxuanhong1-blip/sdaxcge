#!/usr/bin/env bash
# Public GEO matrices for the concordant-4 malignant-cell virtual KO.
...
dl() {
  ...
  echo "GET $url"
  curl -fL --retry 6 --retry-delay 5 --retry-all-errors -o "${dest}.partial" "$url"
...
echo "refused: GSE148071 GSE127465 private 8KL"
ls -lh "$OUT"
