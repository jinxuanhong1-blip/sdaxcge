...
SpatialFDR
----------
Reimplementation of miloR::graphSpatialFDR with weighting='k-distance'
(Dann et al. 2022; Lun et al. cydar). Each neighbourhood p-value is
weighted by the reciprocal of the Euclidean distance from the index cell
to its k-th nearest neighbour in the PCA used to build the graph. Weighted
BH is then:
...
(index sampling and the DA model differ). It is the same SpatialFDR
formula. Report both BH-FDR and SpatialFDR, and the neighbourhood /
sample counts actually tested.

Neighbourhoods
--------------
1. HVG (dispersion of raw UMI among genes with mean in [min_mean, max_mean]).
2. log1p(CP10k) on HVGs, gene-scale, PCA (d dimensions).
3. Subtract the per-sample mean in PCA space (one-step batch centering;
   not Harmony).
4. kNN of every cell.
5. Milo-style refined index sampling (prop random cells → centroid of each
   cell's kNN → nearest member of that kNN becomes the index; unique).
6. Neighbourhood i = {index i} ∪ its k nearest neighbours (size k+1).

DA tests (fallback, not edgeR QLF)
----------------------------------
For neighbourhood i and sample s:

    prop[s, i] = n_cells(s in i) / n_cells(s)

- Binary condition (MPR vs NMPR): Welch t-test of prop across samples.
  logFC = log2(mean_prop_NMPR + eps) − log2(mean_prop_MPR + eps).
- Continuous covariate (sample malignant TACSTD2): Spearman of prop vs score.

A neighbourhood is testable only if it contains cells from enough samples
(see min_samples / min_per_group). Cell-level p-values are never reported.

Composition (TACSTD2-high vs T/NK)
----------------------------------
Transcriptional kNN neighbourhoods are **not spatial niches**. Epithelial
cells cluster with epithelial cells, so an unrestricted TACSTD2 vs T/NK
correlation is largely lineage geometry. Primary composition tests are:

- interface neighbourhoods (≥ min_interface malignant and T/NK cells);
- a greedy low-overlap neighbourhood subset (honest n);
- a sample-paired test: each sample's T/NK fraction among its cells that
  fall in TACSTD2-high vs TACSTD2-low neighbourhoods.
"""
...
def pca_knn(
    ...
    # drop self (first neighbour at distance 0)
...
@dataclass
class Nhoods:
    index: np.ndarray  # (n_nhood,)
    members: np.ndarray  # (n_nhood, k) cell indices; does not include index
    k_distance: np.ndarray  # (n_nhood,) distance of index to its k-th NN
    k: int
...
def nhood_membership(nhoods: Nhoods, n_cells: int) -> list[np.ndarray]:
    """Each neighbourhood as a 1-D array of cell indices (index + k NN)."""
    out = []
...
def attach_fdr(df: pd.DataFrame, k_distance: np.ndarray, p_col: str = "p") -> pd.DataFrame:
    ...
    # SpatialFDR / BH are computed on testable rows only (NA p dropped),
    # matching miloR (NA p-values discarded before weighting).
    out["BH_FDR"] = bh_fdr(p)
        ...
        rows.append(
            {
                ...
                "tacstd2_all_mean": float(tacstd2_log[cells].mean()),
                "tacstd2_malig_mean": float(tacstd2_log[cells[mal]].mean()) if mal.any() else np.nan,
...
def sample_paired_tnk_by_tacstd2(
    members: list[np.ndarray],
    sample: np.ndarray,
    is_tnk: np.ndarray,
    tacstd2_nhood: np.ndarray,
    high: np.ndarray,
    low: np.ndarray,
) -> pd.DataFrame:
    """Per-sample T/NK fraction among cells that sit in TACSTD2-high vs low nhoods."""
...
def self_test() -> None:
    ...
    # identical distances → SpatialFDR equals BH (same weights)
