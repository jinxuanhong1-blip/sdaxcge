...
Triple that DIFFER (PR #459): GSE123902 + GSE131907 + GSE205335.
Not the 7-pool. Not +GSE148071. Not CellChat. Not T/NK infiltrate. No dual-high.

Thesis: CLDN4-high malignant IFN/MHC down, TJ up.

Method: within-cohort CLDN4 %pos Q4 vs Q1, then stacked (cohort covariate).
Patient UMI-sum → TMM → log2(CPM+1) → OLS (limma-style, not muscat).
"""
...
def triple_meta() -> pd.DataFrame:
    """PR #459 locked units: 13 donors + 21 samples + 22 patients = 56."""
    ...
    a["in_count_matrix"] = True
    a["quartile"] = assign_quartiles(a.set_index("patient")["cldn4_pct"]).values

    b = pd.read_csv(DATA / "GSE123902_marker_units.tsv", sep="\t")
    tumor = b[b["tissue"].isin(["PRIMARY", "METASTASIS"])].copy()
    ...
    el["in_count_matrix"] = True
    el["quartile"] = assign_quartiles(el.set_index("patient")["cldn4_pct"]).values

    c = pd.read_csv(DATA / "GSE205335_patients.tsv", sep="\t")
    c["patient"] = c["patient"].astype(str)
    c["cohort"] = "GSE205335"
    c["unit"] = "patient"
    c["malig_def"] = "author_malig"
    c["cldn4_pct"] = c["mal_CLDN4_pct_pos"].astype(float)
    c["cldn4_mean"] = c["mal_CLDN4_mean"].astype(float)
    c["n_malignant"] = c["n_malignant"].astype(int)
    # P4001 (27 malignant cells) is on the PR #459 label vector, not in the UMI-sum matrix
    mal_meta = pd.read_csv(DATA / "GSE205335_malignant_meta.tsv", sep="\t")
    in_mat = set(mal_meta["patient"].astype(str))
    c["in_count_matrix"] = c["patient"].isin(in_mat)
    c["quartile"] = assign_quartiles(c.set_index("patient")["cldn4_pct"]).values

    cols = [
        "patient", "cohort", "unit", "malig_def", "cldn4_pct", "cldn4_mean",
        "n_malignant", "quartile", "in_count_matrix",
    ]
    return pd.concat([el[cols], a[cols], c[cols]], ignore_index=True)
...
def forest_family_scores_stacked(fam_de: pd.DataFrame, title: str, path: Path) -> None:
    """Per-cohort Q4 vs Q1 family scores, then the stacked (combined) row."""
    ...
    if sub.empty:
        return
    fig, ax = plt.subplots(figsize=(8.2, max(5.2, 0.32 * len(sub) + 1.4)))
    y = np.arange(len(sub))
    ax.axvline(0, c="#666", lw=0.7)
    ax.errorbar(
        sub["logFC"],
    ...
    ax.set_yticks(y)
    labels = [
        f"{r.family} · {r.cohort}  n={int(r.n_q1)}/{int(r.n_q4)}  p={r.p:.3g}"
...
def strip_cldn4(meta: pd.DataFrame, path: Path) -> None:
    ...
    fig.suptitle("PR #459 triple CLDN4-only quartiles (within-cohort; black edge = not in malignant matrix)")
    fig.tight_layout()
    fig.savefig(path.with_suffix(".png"), dpi=160)
    fig.savefig(path.with_suffix(".pdf"))
    plt.close(fig)
...
def n_bar(inv: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_ylabel("honest n (donors / samples / patients)")
    ax.set_title("Malignant DE sample sizes — GSE123902+GSE131907+GSE205335 only")
...
def write_finding(
    ...
    n_by = meta.groupby("cohort").size().to_dict()
    ...
    text = f"""# Triple GSE123902+GSE131907+GSE205335 — malignant-cell-intrinsic CLDN4 DE
...
- GSE123902: n={n_by.get('GSE123902', 0)} donors (tumor/met; marker-malignant; normals dropped). Q1={int(q_by['GSE123902'].get('Q1', 0))} Q4={int(q_by['GSE123902'].get('Q4', 0))}.
- GSE131907: n={n_by.get('GSE131907', 0)} samples (tumor origins, n_mal≥20, author malignant). Q1={int(q_by['GSE131907'].get('Q1', 0))} Q4={int(q_by['GSE131907'].get('Q4', 0))}.
- GSE205335: n={n_by.get('GSE205335', 0)} patients (author malignant). Q1={int(q_by['GSE205335'].get('Q1', 0))} Q4={int(q_by['GSE205335'].get('Q4', 0))}. Dropped from malignant DE (not in UMI-sum): {drop_txt}.

| contrast | n_low / n_high | n_genes | note |
|---|---:|---:|---|
| Q4 vs Q1 stacked | {int(mal.get('n_low', 0))}/{int(mal.get('n_high', 0))} | {int(mal.get('n_genes', 0) or 0)} | cohort covariates; malignant only |
| continuous stacked | n={int(mal_c.get('n', 0) or 0)} | {int(mal_c.get('n_genes', 0) or 0)} | CLDN4 %pos z |
...
def main() -> None:
    ...
    if not de.empty:
        ...
        forest_families(
            de,
            fam,
            "malignant family genes, Q4 vs Q1 stacked",
    ...
    if not fam_score.empty:
        ...
        forest_family_scores(
            head_fs,
            "malignant family scores, Q4 vs Q1 stacked",
            FIGS / "forest_family_scores_q4q1",
        )
        forest_family_scores_stacked(
            fam_score,
            "within-cohort Q4 vs Q1 family scores, then stacked",
            FIGS / "forest_family_scores_stacked",
        )
