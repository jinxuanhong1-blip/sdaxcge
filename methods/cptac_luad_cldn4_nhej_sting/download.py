#!/usr/bin/env python3
"""Download CPTAC LUAD public TMT protein + phenotype (freeze v1.2).

Open HTTP only. No dbGaP / CDS tokens. Files >2 GB are skipped.
Protein: gene-abundance tumor TMT (log2 reference intensity; TMT dropout kept as NA).
Phenotype: WES/WGS purity for a pre-specified residual. Not an ICI table.
"""
...
def head_length(url: str) -> int | None:
    req = urllib.request.Request(url, method="HEAD", headers={"User-Agent": "cptac-luad-cldn4-nhej-sting/1.0"})
...
def fetch(url: str, dest: Path) -> dict:
    ...
    try:
        ...
        if dest.exists() and nbytes is not None and dest.stat().st_size == nbytes:
            ...
            req = urllib.request.Request(url, headers={"User-Agent": "cptac-luad-cldn4-nhej-sting/1.0"})
...
def main() -> int:
    ...
    p.add_argument("--outdir", default="data/cptac_luad_cldn4_nhej_sting")
    args = p.parse_args()
    ...
    manifest = {
        ...
        "freeze_base": FREEZE,
        "citation": "Gillette et al. Cell 2020; treatment-naive surgical LUAD; no ICI labels",
        "scope": "CLDN4 protein vs DNA-PKcs/Ku/STING/TBK1/IRF3/HLA proteins",
