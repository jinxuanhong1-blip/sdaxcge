...
def write_finding(corr: pd.DataFrame, quart: pd.DataFrame, counts: pd.DataFrame, geneset_rows: pd.DataFrame) -> None:
    ...
    lines.append("")
    lines.append("## Specification sweep")
    lines.append("")
    lines.append("An exploratory grid over cutoffs, gene sets, ssGSEA / GSVA / AUCell / z-mean, covariates, and cohort strata is in `SWEEP.md`. It does not replace the pre-specified tests above. No quadruple in that grid is significant in the thesis direction for NHEJ together with IFN, STING, and APM.")
    lines.append("")
    lines.append("## Files")
    lines.append("")
