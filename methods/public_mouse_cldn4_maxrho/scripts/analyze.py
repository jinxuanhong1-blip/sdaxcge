#!/usr/bin/env python3
"""Mouse-level Cldn4% vs T fraction and vs IFN/APM on the public integrate cohorts.

Cohorts (only): GSE154977, GSE180963, GSE165641.
Private 8 KL matrices are not read and are not merged.

Harmony is not re-fit. Cldn4 percent, T fraction, and IFN/APM are marker
summaries of the processed counts. The epithelial caller never uses Cldn4.
...
Selection rule, fixed before any correlation is ranked:
  ...
  Headline exposures are Cldn4 percents (count > 0, count >= 2, count >= 5,
...
MARKERS = sorted(set(
    ["Cldn4", "Ptprc", "Sftpc", "Nkx2-1", "Scgb1a1", "Ager", "Cldn18"]
...
def mouse_table_for_gate(gate, counts, lognorm, meta, gene_index, cldn4_i, ifn, apm, mhc) -> pd.DataFrame:
    is_epi = gate
    ...
    cldn4_count = counts[cldn4_i]
    cldn4_log = lognorm[cldn4_i]
    rows = []
...
def eligible_mice(df: pd.DataFrame, min_epi: int, yname: str) -> tuple[pd.DataFrame, str]:
    ...
    if yname in Y_T:
        ...
        low_t = mixed & ((out["n_tnk"] < MIN_TNK_CELLS) | (out["frac_tnk"] < MIN_TNK_FRAC))
        if (keep & low_t).any():
            reasons.append("below_tnk_floor")
            keep &= ~low_t
    out = out.loc[keep]
...
def _rank_frame(sub: pd.DataFrame) -> pd.DataFrame:
    ...
    gate and Cldn4 count>0, then an unfiltered genotype and treatment, then
    more mice.
    """
    out = sub.copy()
...
def validate_locked(table: pd.DataFrame) -> pd.DataFrame:
    ...
    for mouse, ref in PUBLISHED.items():
        ...
        rows.append(dict(
            ...
            pct_gt0=float(r["pct_gt0"]),
            pct_published=ref["pct"],
            pct_delta=float(r["pct_gt0"]) - ref["pct"],
...
def write_finding(path: Path, winners: dict[str, pd.DataFrame], eligible: pd.DataFrame,
    ...
    def block(family: str) -> str:
        ...
        lines = [
            ...
            "The row above is the ceiling tie that prefers the locked gate and Cldn4 count>0. "
    ...
    val_ok = bool((validation["n_epi_delta"].abs() <= 2).all() and (validation["pct_delta"].abs() < 0.2).all())
    text = f"""# Public integrate cohorts: maximum |ρ| for mouse-level Cldn4%
...
Locked-gate replay against the integrate mouse table: {"n_epi and Cldn4% match" if val_ok else "MISMATCH — see tables/locked_gate_validation.tsv"}. T/NK fractions match that table. Epithelial IFN means are lower by a nearly constant factor (median my/published = {summary["ifn_scale_median"]:.3f}) because this run log-normalizes with each cell's full UMI total. The integrate object normalized after the cross-study gene intersect. The eight-mouse rank order of locked-gate IFN is unchanged, and Spearman of Cldn4% vs that published IFN is the same value, −0.119.
...
def scatter(path: Path, mice: pd.DataFrame, row: pd.Series, title: str) -> None:
    ...
    ax.set_xlabel(f"epithelial Cldn4% ({row['x']})")
    ax.set_ylabel(row["y"])
...
def main() -> None:
    ...
    if "Cldn4" not in gene_index:
        raise RuntimeError("Cldn4 missing after intersect")
    ...
    for name, mask in gates.items():
        ...
        table = mouse_table_for_gate(mask, counts, lognorm, meta, gene_index, cldn4_i, ifn, apm, mhc)
    ...
    if validation["n_epi_delta"].abs().max() > 5 or validation["pct_delta"].abs().max() > 1.0:
