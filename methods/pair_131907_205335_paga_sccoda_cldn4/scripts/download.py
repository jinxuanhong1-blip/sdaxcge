...
Skip the 2.86 GB GSE131907 log2TPM text matrix, EGA/FASTQ, GSE148071,
and GSE207422. Only raw UMI + author annotations.
"""
...
def manifest(root: Path) -> None:
    ...
    out.write_text(
        json.dumps(
            {"accessions": ["GSE131907", "GSE205335"], "skipped": SKIPPED, "files": rows},
            indent=2,
        )
    )
...
def main() -> None:
    ...
    p.add_argument("--out", type=Path, default=Path("/tmp/pair_131907_205335"))
    args = p.parse_args()
