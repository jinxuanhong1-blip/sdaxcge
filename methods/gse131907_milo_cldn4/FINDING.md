...
**tLung (primary): 0 neighbourhoods at SpatialFDR < 0.1 or < 0.2.** Median-split
Welch (5 vs 5) is also empty. That is the result, not a skipped test.

mBrain (secondary, same-site) has **2** neighbourhoods at SpatialFDR = 0.061
(none < 0.05). Both are myeloid/B, 0 T/NK, 1–2 malignant cells. They are not
interface nhoods and they do not support a T/NK-poor-next-to-CLDN4-high-epithelium
claim.
...
|ρ|=1 uses the exact two-sided tail 2/*n!*, not scipy’s t-approximation
(which reports p≈0).

Public matrix: 208,506 cells, 29,634 genes (raw UMI text). Graph cells are
epithelium + immune at one site.
...
Do not cite 42,776 graph cells or 2,998 neighbourhoods as *n*. Neighbourhoods overlap. A disjoint subset has n=266. Only 210 / 2,998 neighbourhoods are testable (cells from ≥5 scored samples); size-31 kNN after per-sample PCA centering is often sample-private.
...
|---|---|---|---|---|---|---|---|
| Malignant CLDN4 Spearman | 210 | 17 | 1.37e-02 | 0.432 | **0** | 0 | 0 |
...
| Contrast | Arm | n samples | Why this n |
|---|---|---|---|
| Malignant CLDN4 (continuous Spearman) | samples with ≥10 author-malignant cells | **10** of 10 | Mean log1p-CP10k CLDN4 in author malignant cells. Dropped if <10 (none). |
| Median split (Welch, secondary) | high / low | **5 vs 5** | Median of the scored samples. Ties at the median unlabeled. |

Do not cite 27,708 graph cells or 1,956 neighbourhoods as *n*. Neighbourhoods overlap. A disjoint subset has n=192. Only 26 / 1,956 neighbourhoods are testable.
...
| Contrast | testable | P<0.05 | min P | min SpatialFDR | SpatialFDR<0.1 | SpatialFDR<0.05 | BH<0.1 |
|---|---|---|---|---|---|---|---|
| Malignant CLDN4 Spearman | 26 | 3 | 2.78e-03 | 0.061 | **2** | 0 | 2 |
| Median split high vs low | 10 | 0 | 1.98e-01 | 0.614 | **0** | 0 | 0 |

The two SpatialFDR 0.061 nhoods (nhoods 155 and 744) have 0 T/NK cells and 1–2 malignant cells (dominant lineage myeloid / B). Median-split DA on the same graph is empty.

Sample-level malignant CLDN4 vs T/NK fraction (no neighbourhoods): ρ=-0.697, p=2.51e-02, n=10. That is a 10-sample composition correlation, not neighbourhood DA.
...
- The two mBrain SpatialFDR 0.061 nhoods are not a replication of a tLung
  neighbourhood DA (tLung is empty) and are not T/NK-interface nhoods.
