#!/usr/bin/env python3
"""NicheNet-style ligand activity: TACSTD2-high/CLDN4-high malignant -> T/NK.
...
def main() -> int:
    ...
    # NicheNet matrix: targets in rows, ligands in columns
    if lt.shape[0] < lt.shape[1]:
        # unusual; leave as-is but log
    ...
    # DRMref response already present; prefer GEO sheet
    ...
    # High-barrier: both TACSTD2 and CLDN4 >= malignant-cell median (pre-registered)
    med_t = float(mal["log_TACSTD2"].median()) if "log_TACSTD2" in mal else np.nan
    med_c = float(mal["log_CLDN4"].median()) if "log_CLDN4" in mal else np.nan
    mal["high_barrier"] = (mal["log_TACSTD2"] >= med_t) & (mal["log_CLDN4"] >= med_c)
    mal["barrier_z"] = stats.zscore(mal["log_TACSTD2"].to_numpy(), nan_policy="omit") + stats.zscore(
        mal["log_CLDN4"].to_numpy(), nan_policy="omit"
    )
    n_high = int(mal["high_barrier"].sum())
    print(f"high-barrier malignant {n_high}/{len(mal)} med_TACSTD2={med_t:.3f} med_CLDN4={med_c:.3f}", flush=True)
    ...
    for pid, g in post.groupby("patient"):
        ...
        rec = {
            ...
            "n_high_barrier": int(mg["high_barrier"].sum()) if len(mg) else 0,
            "n_tnk": int(len(tg)),
    ...
    for x, y in [
        ("mal_barrier", "tnk_cyto"),
        ("mal_barrier", "tnk_exh"),
        ("mal_barrier", "tnk_exh_minus_cyto"),
        ("mal_TACSTD2", "tnk_cyto"),
        ("mal_TACSTD2", "tnk_exh"),
        ("mal_CLDN4", "tnk_cyto"),
        ("mal_CLDN4", "tnk_exh"),
        ("mal_barrier", "tnk_frac"),
    ]:
    ...
    high = mal[mal["high_barrier"]]
    low = mal[~mal["high_barrier"]]
    ...
    print(
        f"expressed ligands high-barrier={len(expr_lig_high)} low={len(expr_lig_low)} "
        f"T/NK receptors={len(expr_rec_tnk)} potential={len(potential)}",
        flush=True,
    )

    # combinatorial senders
    ...
    # background = genes in prior that are expressed in T/NK (10% rule) or present in panel+prior
    tnk_expr = expressed_genes(tnk, list(lt.index.intersection(panel.columns)))
    background = sorted(set(tnk_expr) | set(CYTOTOXICITY) | set(EXHAUSTION))
    background = [g for g in background if g in lt.index]
    # NicheNet typically uses a broader background: all expressed receiver genes in the prior
    # We only extracted a panel, so background is panel ∩ prior ∩ T/NK-expressed.
    # Honest: this is a restricted background (not the full transcriptome).
    print(f"background genes (panel-restricted) {len(background)}", flush=True)

    # empirical patient-level DE of T/NK programs (gene-wise Wilcoxon on patient means)
    gene_de = []
    tnk_genes = [g for g in (CYTOTOXICITY + EXHAUSTION + list(set(tnk_expr))) if f"log_{g}" in tnk.columns]
    ...
    sets = {
        "a_priori_exhaustion": set(EXHAUSTION) & set(lt.index),
    ...
    for set_name, gs in sets.items():
        for setting, ligs in [
            ("high_barrier_to_all_TNK", potential),
            ("high_barrier_NMPR_to_NMPR_TNK", pot_nmpr),
            ("high_barrier_MPR_to_MPR_TNK", pot_mpr),
        ]:
    ...
    # top ligands per setting × geneset
    tops = []
    if not act_all.empty:
    ...
    # ligand expression in high-barrier malignant, patient-level (for honest n)
    lig_pat = []
    ...
    for pid, g in high.groupby("patient"):
        rec = {"patient": pid, "mpr_group": g["mpr_group"].iloc[0], "n_high_barrier": int(len(g))}
        for L in focus_ligands:
            if f"log_{L}" in g.columns:
                rec[L] = float(g[f"log_{L}"].mean())
        lig_pat.append(rec)
    lig_pat_df = pd.DataFrame(lig_pat)
    lig_pat_df.to_csv(OUT / "high_barrier_ligand_patient_means.tsv", sep="\t", index=False)

    # NMPR vs MPR on high-barrier ligand means (patient-level)
    lig_tests = []
    ...
    if not lig_test_df.empty:
        ...
        lig_test_df.to_csv(OUT / "high_barrier_ligand_NMPR_vs_MPR.tsv", sep="\t", index=False)

    # patient-level ligand vs T/NK program (n=12)
    lig_corr = []
    merged = lig_pat_df.merge(post_s[["patient", "tnk_cyto", "tnk_exh", "tnk_exh_minus_cyto"]], on="patient")
    for L in focus_ligands:
        if L not in merged.columns:
            continue
        for y in ["tnk_cyto", "tnk_exh", "tnk_exh_minus_cyto"]:
    ...
    n_tbl = pd.DataFrame(
        [
            ...
            {"item": "high_barrier_malignant", "n": n_high, "note": "TACSTD2 and CLDN4 >= malignant median"},
            {"item": "high_barrier_MPR_cells", "n": int(len(high_mpr)), "note": "not a patient replicate"},
            {"item": "high_barrier_NMPR_cells", "n": int(len(high_nmpr)), "note": "not a patient replicate"},
            ...
            {"item": "potential_ligands_pooled", "n": int(len(potential)), "note": "high-barrier ligand + T/NK receptor"},
    ...
    def top_list(setting: str, geneset: str, k: int = 10) -> list[dict]:
        ...
        return [
            {"rank": int(r.rank), "ligand": r.ligand, "pearson": float(r.pearson), "auroc": float(r.auroc)}
    ...
    summary = {
        ...
        "n": {
            ...
            "high_barrier_malignant": n_high,
            "tnk_cells": int(len(tnk)),
            "potential_ligands": int(len(potential)),
        },
        "barrier_cutoffs": {"median_log1p_cp10k_TACSTD2": med_t, "median_log1p_cp10k_CLDN4": med_c},
        "hypothesis_patient_tests": {
            ...
            in {
                ...
                "NMPR_vs_MPR_tnk_exh",
                "NMPR_vs_MPR_tnk_exh_minus_cyto",
                "spearman_mal_barrier__tnk_cyto",
                "spearman_mal_barrier__tnk_exh",
            }
        },
        "empirical_sets": {
            "up": sorted(emp_up),
        ...
        "top_ligands": {
            "high_barrier_to_all_TNK__exhaustion": top_list(
                "high_barrier_to_all_TNK", "a_priori_exhaustion"
            ),
            "high_barrier_to_all_TNK__cytotoxicity": top_list(
                "high_barrier_to_all_TNK", "a_priori_cytotoxicity"
            ),
            "NMPR_to_NMPR__exhaustion": top_list(
                "high_barrier_NMPR_to_NMPR_TNK", "a_priori_exhaustion"
            ),
            "MPR_to_MPR__exhaustion": top_list(
                "high_barrier_MPR_to_MPR_TNK", "a_priori_exhaustion"
            ),
            "NMPR_to_NMPR__empirical_up": top_list(
                "high_barrier_NMPR_to_NMPR_TNK", "empirical_NMPR_TNK_up"
            ),
            "NMPR_to_NMPR__empirical_down": top_list(
                "high_barrier_NMPR_to_NMPR_TNK", "empirical_NMPR_TNK_down"
            ),
        },
        "caveats": [
    ...
    for ax, metric, title in [
        (axes[0], "mal_barrier", "Malignant TACSTD2/CLDN4"),
        (axes[1], "tnk_cyto", "T/NK cytotoxicity"),
        (axes[2], "tnk_exh", "T/NK exhaustion"),
    ]:
        for grp, sub in post_s.groupby("mpr_group"):
            ax.scatter(
                np.random.default_rng(0).uniform(-0.08, 0.08, len(sub)) + (0 if grp == "MPR" else 1),
                sub[metric],
                c=colors[grp],
                s=40,
    ...
    for grp, sub in post_s.groupby("mpr_group"):
        ax.scatter(sub["mal_barrier"], sub["tnk_exh_minus_cyto"], c=colors[grp], s=50, label=f"{grp} n={len(sub)}")
        for r in sub.itertuples():
            ax.annotate(r.patient, (r.mal_barrier, r.tnk_exh_minus_cyto), fontsize=7, xytext=(3, 3), textcoords="offset points")
    ax.set_xlabel("Malignant (TACSTD2+CLDN4)/2")
    ax.set_ylabel("T/NK exhaustion − cytotoxicity")
    ax.legend(frameon=False, fontsize=8)
    fig.tight_layout()
    fig.savefig(OUT / "fig2_barrier_vs_exh_minus_cyto.png", dpi=160)
    plt.close(fig)

    if not top.empty:
        fig, axes = plt.subplots(1, 2, figsize=(11, 5.2))
        for ax, gs, title in [
            (axes[0], "a_priori_exhaustion", "Prior activity vs exhaustion set"),
            (axes[1], "a_priori_cytotoxicity", "Prior activity vs cytotoxicity set"),
        ]:
            sub = top[(top["setting"] == "high_barrier_to_all_TNK") & (top["geneset"] == gs)].head(12)
            if sub.empty:
                ax.axis("off")
                continue
        ...
        # combinatorial comparison of ranks
        a = act_all[
            (act_all["geneset"] == "a_priori_exhaustion")
            & (act_all["setting"] == "high_barrier_NMPR_to_NMPR_TNK")
        ][["ligand", "pearson"]].rename(columns={"pearson": "pearson_NMPR"})
        b = act_all[
            (act_all["geneset"] == "a_priori_exhaustion")
            & (act_all["setting"] == "high_barrier_MPR_to_MPR_TNK")
        ][["ligand", "pearson"]].rename(columns={"pearson": "pearson_MPR"})
        both = a.merge(b, on="ligand", how="inner")
        if not both.empty:
            both.to_csv(OUT / "combinatorial_exhaustion_activity_MPR_vs_NMPR.tsv", sep="\t", index=False)
            ...
            ax.set_title("Combinatorial: same prior, MPR vs NMPR T/NK")
            fig.tight_layout()
