...
No FASTQ. No EGA. GSE189357 RAW tar is optional if malignant counts
...
def main() -> None:
    ...
    p.add_argument(
        "--skip-189357",
        action="store_true",
        help="skip GSE189357 RAW tar (use committed malignant counts)",
    )
    args = p.parse_args()
    for name, url in FILES.items():
        if args.skip_189357 and name.startswith("gse189357/"):
