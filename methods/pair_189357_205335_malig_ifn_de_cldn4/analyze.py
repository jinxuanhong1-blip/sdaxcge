...
Pair: GSE189357 (marker malignant, patient) + GSE205335 (author malignant, patient).
PR #459 given cut: %pos n=31 ρ=−0.478, Q4 r=−0.750. Not CellChat.
Not T/NK infiltrate. No dual-high. No GSE148071.
...
def pair_meta() -> pd.DataFrame:
    """PR #459 locked units: GSE189357 n=9 + GSE205335 n=22 patients."""
    a = pd.read_csv(DATA / "GSE205335_patients.tsv", sep="\t")
    a["patient"] = a["patient"].astype(str)
    a["cohort"] = "GSE205335"
    a["unit"] = "patient"
    a["malig_def"] = "author_malig"
    a["cldn4_pct"] = a["mal_CLDN4_pct_pos"].astype(float)  # already 0–100
    a["cldn4_mean"] = a["mal_CLDN4_mean"].astype(float)
    a["n_malignant"] = a["n_malignant"].astype(int)
    a["histology"] = a["cancer_subtype"].astype(str)
        ...
        rows.append(
            {
                "family": name,
                "split": split,
                "cohort": cohort or "GSE189357+GSE205335",
...
def strip_cldn4(meta: pd.DataFrame, path: Path) -> None:
    ...
    fig.suptitle("PR #459 pair CLDN4-only quartiles (GSE189357 + GSE205335 patients)")
    fig.tight_layout()
    fig.savefig(path.with_suffix(".png"), dpi=160)
    fig.savefig(path.with_suffix(".pdf"))
    plt.close(fig)
...
def scatter_family_vs_cldn4(
    logcpm: pd.DataFrame, meta: pd.DataFrame, fam: dict[str, set[str]], path: Path
) -> None:
    """Extra figure: family score vs malignant CLDN4 %pos (all 31 patients)."""
    m = meta.loc[meta["patient"].isin(logcpm.columns)].copy()
    names = [k for k in ("IFN", "MHC-I/APM", "TJ") if any(g in logcpm.index for g in fam[k])]
    if not names:
        return
    fig, axes = plt.subplots(1, len(names), figsize=(3.4 * len(names), 3.6), squeeze=False)
    cols = {"GSE189357": "#4c78a8", "GSE205335": "#e45756"}
    marks = {"Q1": "o", "Q2": "s", "Q3": "D", "Q4": "^"}
    for i, name in enumerate(names):
        ax = axes[0][i]
        present = [g for g in fam[name] if g in logcpm.index]
        score = logcpm.loc[present, m["patient"]].astype(float).mean(axis=0)
        for r in m.itertuples():
            ax.scatter(
                r.cldn4_pct,
                score.loc[r.patient],
                c=cols.get(r.cohort, "#333"),
                marker=marks.get(r.quartile, "o"),
                s=36,
                edgecolors="white",
                linewidths=0.4,
                zorder=3,
            )
        x = m["cldn4_pct"].astype(float).to_numpy()
        y = score.loc[m["patient"]].to_numpy()
        if len(x) >= 5:
            rho, p = stats.spearmanr(x, y)
            ax.set_title(f"{name}\nρ={rho:+.3f} p={p:.3g}", fontsize=9)
        else:
            ax.set_title(name, fontsize=9)
        ax.set_xlabel("malignant CLDN4 %pos")
        ax.set_ylabel("mean log2(CPM+1)")
    for cohort, c in cols.items():
        axes[0][0].scatter([], [], c=c, label=cohort, s=28)
    axes[0][0].legend(frameon=False, fontsize=7, loc="best")
    fig.suptitle("Family score vs CLDN4 %pos (patient units; not T/NK)", y=1.03)
    fig.tight_layout()
    fig.savefig(path.with_suffix(".png"), dpi=160, bbox_inches="tight")
    fig.savefig(path.with_suffix(".pdf"), bbox_inches="tight")
    plt.close(fig)
...
def n_bar(inv: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_ylabel("honest n (patients)")
    ax.set_title("Malignant DE sample sizes — GSE189357+GSE205335 only")
...
def write_finding(
    ...
    def score_md(split: str, cohort: str = "GSE189357+GSE205335") -> str:
    ...
    def score_md_cohorts(split: str) -> str:
        sub = fam_score[
            (fam_score["split"] == split) & (fam_score["cohort"] != "GSE189357+GSE205335")
        ]
        if sub.empty:
    ...
    q205_hist = (
        meta[(meta["cohort"] == "GSE205335") & (meta["quartile"] == "Q4")]["histology"]
        .value_counts()
        .to_dict()
    )
    q205_hist_txt = ", ".join(f"{k}={v}" for k, v in sorted(q205_hist.items())) or "none"
    cldn4_txt = (
    ...
    text = f"""# Pair GSE189357+GSE205335 — malignant-cell-intrinsic CLDN4 DE
...
- GSE189357: n={n189} patients (marker-malignant; one 10x sample / patient). Q1={int(q189.get('Q1',0))} Q4={int(q189.get('Q4',0))}.
- GSE205335: n={n205} patients (author `Malignant cells`; 4 extra GEO patients with 0 malignant already out). Q1={int(q205.get('Q1',0))} Q4={int(q205.get('Q4',0))}. GSE205335 Q4 histology: {q205_hist_txt}.
...
def main() -> None:
    ...
    for cohort in (None, "GSE189357", "GSE205335"):
    ...
    scatter_family_vs_cldn4(
        lc_all,
        m_all,
        fam,
        FIGS / "scatter_family_vs_cldn4",
    )
    ...
    if not fam_score.empty:
        ...
        head_fs = fam_score[
            (fam_score["cohort"] == "GSE189357+GSE205335") & (fam_score["split"] == "q4q1")
        ]
