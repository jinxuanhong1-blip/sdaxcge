...
def pca_knn(
    ...
    """Return (pca, knn_idx [n,k] excluding self, knn_dist).

    Per-sample PCA mean centering is the per-dataset batch step (not Harmony).
    Set center_by_sample=False when the embedding is already Harmony-corrected.
    """
...
def knn_from_embedding(
    embedding: np.ndarray,
    k: int = 30,
) -> tuple[np.ndarray, np.ndarray]:
    """kNN on a precomputed embedding (Harmony-corrected PCs)."""
    nn = NearestNeighbors(n_neighbors=k + 1, algorithm="auto", metric="euclidean")
    nn.fit(embedding)
