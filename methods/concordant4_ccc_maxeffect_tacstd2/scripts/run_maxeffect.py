...
TACSTD2-high vs TACSTD2-low malignant senders -> T/NK (TROP2+ barrier face).
...
Does not replace the locked CLDN4 max-effect table (PR 713) or the official
CellChat CLDN4 family sum (PR 616, +0.0037). Same score menu, TACSTD2 gate.
"""
...
def wanted_genes() -> set[str]:
    genes = set(EPI + TNK_MARKERS + [GATE, "CLDN4", "PTPRC"])
...
def qc_counts(units: list[dict]) -> pd.DataFrame:
    ...
    for u in units:
        comp = u["comp"]
        mal = comp == "MAL"
        gate_i = u["genes"].index(GATE) if GATE in u["genes"] else None
        if gate_i is not None and mal.any():
            ln = np.log1p(u["mat"][mal, gate_i] / np.maximum(u["lib"][mal], 1.0) * 1e4)
            mean_ln = float(ln.mean())
            pct = float(np.mean(u["mat"][mal, gate_i] > 0))
        else:
...
def score_unit(unit: dict) -> list[dict]:
    ...
    if mal.size < MIN_MAL or tnk.size < MIN_TNK or GATE not in unit["genes"]:
        return []
    genes = unit["genes"]
    ...
    logv = np.log1p(cp)
    gate = logv[mal, gindex[GATE]]
    # Flat TACSTD2 (e.g. EBUS_13: no malignant counts) cannot define a high/low arm.
    if float(np.nanmax(gate) - np.nanmin(gate)) <= 0:
        return []
    rows = []
    # stats cache per split
    for mode in ("q4q1", "decile"):
        split = arms(gate, mode)
        if split is None:
            continue
        high_m, low_m = split
        if float(gate[high_m].mean()) <= float(gate[low_m].mean()):
            # Skip tied/flat arms rather than inventing a contrast.
            continue
        high = mal[high_m]
        low = mal[low_m]
...
def select_winner(summary: pd.DataFrame, lig: pd.DataFrame) -> dict:
    ...
    # MAX EFFECT for PPT TROP2+ barrier face: among scores with all four barrier
    # ligands positive in 4/4 cohorts, take the largest absolute family sum.
    # Chemokine-family 4/4 (usually CXCL16) is reported honestly but does not
    # demote the percentage-point score — same absolute-Δ spirit as CLDN4 PR 713.
    tiers = [
        base & merged["lig4"],
    ...
    tier_name = [
        "4/4 ligands each 4/4 cohorts (max absolute family sum)",
...
def plot_winner(fam: pd.DataFrame, method: str, split: str, path: Path) -> None:
    ...
    fig.suptitle(f"{method} · {split} · TACSTD2-high minus TACSTD2-low → T/NK", fontsize=11)
    fig.tight_layout()
...
def write_finding(summary, ranked, winner, tier, lig, pair7_sum, inv) -> None:
    ...
    lines = []
    lines.append("# FINDING — largest TACSTD2 (TROP2+) barrier-ligand Δ, concordant-4")
    lines.append("")
    lines.append("ADDITIVE. TACSTD2 gate for PPT **TROP2+ barrier face** (Part1).")
    lines.append("Does not replace the locked CLDN4 max-effect table (PR 713, +27.8 pp) or CellChat PR 616 (+0.0037).")
    lines.append("No dual-high. No CLDN4 gate here. Concordant four only.")
    lines.append("GSE148071, GSE127465, GSE154826, GSE200563, and E-MTAB-13526 are not added.")
    lines.append("Senders are malignant TACSTD2-high vs TACSTD2-low. Receiver is T/NK.")
    ...
    if split != "q4q1" and not q4.empty:
        ...
        lines.append("")
        lines.append(
            f"Q4 vs Q1, same score (locked gate style): n={int(q['n'])}, per-ligand mean {fmt_num(q['mean_per_ligand'])}, "
    ...
    lines.append("")
    lines.append("## Why CellChat population-scaled Δ stays tiny")
    lines.append("")
    lines.append("Official CellChat with `population.size=TRUE` multiplies every edge by sender × receiver proportions, so a real ligand gap becomes a probability near zero (CLDN4 PR 616 family sum +0.0037). The same seven pairs on the TACSTD2 gate, recomputed here:")
    lines.append("")
    ...
    lines.append("")
    lines.append("- This is an expression ligand–receptor contrast. It is not spatial exclusion and it does not replace CosMx TACSTD2 short-range cold (PR 726/735) or the locked CLDN4 CosMx 8/8.")
    lines.append("- Population-scaled CellChat probabilities are not re-issued as +0.0037. The pair-sum table above is the recomputed Hill score on the TACSTD2 gate.")
    ...
    lines.append("- CLDN4 is not the gate here. No dual-high. No mediation claim. GSE148071 is not added.")
    lines.append("- Part1 slide claim is TROP2+ malignant cells carry a stronger barrier-ligand face toward T/NK; immune exclusion ρ stays on CLDN4 %pos (PR 539/712).")
    lines.append("")
    lines.append("## Reproduce")
    lines.append("")
    lines.append("```bash")
    lines.append("bash methods/concordant4_ccc_maxeffect_tacstd2/scripts/download.sh /tmp/concordant4_raw")
    lines.append("python3 methods/concordant4_ccc_maxeffect_tacstd2/scripts/prepare_131907.py")
    lines.append("gcc -O3 -o /tmp/extract_131907 methods/concordant4_ccc_maxeffect_tacstd2/scripts/extract_131907.c -lz")
    lines.append("/tmp/extract_131907 \\")
    lines.append("  /tmp/concordant4_raw/GSE131907/GSE131907_Lung_Cancer_raw_UMI_matrix.txt.gz \\")
    lines.append("  /tmp/concordant4_cache/gse131907/genes_request.txt \\")
    lines.append("  /tmp/concordant4_cache/gse131907/keep_idx.i32 \\")
    lines.append("  208506 \\")
    lines.append("  /tmp/concordant4_cache/gse131907")
    lines.append("python3 methods/concordant4_ccc_maxeffect_tacstd2/scripts/run_maxeffect.py")
    lines.append("```")
    lines.append("")
    text = "\n".join(lines) + "\n"
