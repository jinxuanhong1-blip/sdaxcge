# Finding — Seurat GSE179502 Lkb1 restore, Cldn4-only (mouse)

ADDITIVE. **Cldn4 only.** No dual-high Tacstd2×Cldn4. No human. No GSE179501
(unsorted total cells). No private 8 KL. Public Cell Ranger aggr only:
`GSE179502_XTR_sorted_scRNAseq_{matrix.mtx,features.tsv,barcodes.tsv}.gz`
(Murray / Winslow *Nat Commun* 2022, PMID 35228570).

**Engine: R + Seurat 5.5.1 `CreateSeuratObject`.** This is not a Python/pandas
score. `analyze.R` stops if Seurat is missing.
...
independent). Exact two-sided MWU at 3 vs 3 cannot go below **p=0.1**.
...
IFN/MHC-cold: mean-log IFN/MHC is flat-to-up, and the IFN Spearman vs Cldn4
collapses to null after UMI. Seurat `AddModuleScore` IFN can flip slightly
negative after control-gene subtraction — that is not an IFN-cold call.

## Seurat object (actually created)

| item | value |
|---|---|
| `CreateSeuratObject` | **yes** (Seurat 5.5.1 / SeuratObject 5.4.0) |
| genes × barcodes on GEO mtx | 31,055 × **16,017** |
| QC (UMI≥500, genes≥200, mito≤20%) | **12,643** cells |
| reductions computed | PCA + UMAP (uwot) |
| proof | `tables/seurat_object_proof.rds` |
...
| QC cells | **12,643** |
| Exact two-sided MWU floor at 3 vs 3 | **0.1** |

## Restore contrast (mouse means; Restored − NonRestored)

Score = mean Seurat `LogNormalize` (log1p CP10k) unless marked AMS
(`AddModuleScore`). R `wilcox.test` default (normal approx) writes ~0.081
when ranks fully separate; the exact MWU at 3 vs 3 is **0.1**.

| feature | NonRestored mean | Restored mean | Δ | Welch p | exact MWU | rank-biserial |
|---|---:|---:|---:|---:|---:|---:|
| **Cldn4** | 0.300 | 0.143 | **−0.157** | **0.036** | **0.1** | **−1.0** |
| frac Cldn4+ | 0.283 | 0.139 | −0.144 | 0.068 | 0.1 | −1.0 |
| **AT2** (14/14) | 2.155 | 2.665 | **+0.510** | **0.022** | **0.1** | **+1.0** |
| AT2 AMS | 1.092 | 1.640 | +0.548 | 0.009 | 0.1 | +1.0 |
...
## Per-mouse scores (QC cells; Seurat log1p CP10k means)
...
| AT2 AMS | Q4 vs Q1 | 3160 / 3160 | −1.022 | −0.30 | −0.46 | −0.44 |
| TJ A8 AMS | Q4 vs Q1 | 3160 / 3160 | +0.050 | +0.41 | +0.48 | +0.40 |

Cldn4+ vs Cldn4− is the same sign on the mean-log scores.
...
| IFN A8 AMS | −0.010 | **6** / 0 | 0.031 |
| AT2 AMS | −1.073 | **6** / 0 | 0.031 |
| TJ A8 AMS | +0.056 | 0 / **6** | 0.031 |

Wilcoxon two-sided floor at 6/6 same sign is **p=0.031**. Mean-log IFN/MHC
are slightly **up** in Cldn4-high cells in every mouse — the opposite of
“IFN/MHC down.” The IFN effect is tiny and UMI-sensitive. TJ up and AT2
down are large and stable on both mean-log and AMS.
...
- Public GEO mtx + features + barcodes only. `Seurat::Read10X` →
  `CreateSeuratObject`. Barcode prefix = mouse (CM0875, CM0879, CM0884,
  ZR1932, ZR1966, ZR1969). Cohort / treatment / genotype from the GSE179502
  sample record (`data/samples.csv`).
- QC: UMI ≥500, genes ≥200, mito (`mt-*`) ≤20% via `PercentageFeatureSet`.
...
- Modules: Seurat `AddModuleScore` (`*_AMS`) and mean of member genes on
  the data layer (`*_mean`, same units as Cldn4).
...
- Figures: Seurat `VlnPlot`, `DotPlot`, `DimPlot` (PCA + UMAP), `FeaturePlot`.
- Cldn4 only. No dual-high. No human. No GSE179501. No CellChat. No private 8 KL.
- Reproduce: `Rscript methods/seurat_gse179502_cldn4/analyze.R`

## Files

- `analyze.R` / `gene_sets.R` — GEO cache + `CreateSeuratObject` + scores
...
- `tables/seurat_object_proof.rds` — class, versions, n, reductions
- `figures/vln_cldn4_by_restore.{png,pdf}` — Seurat `VlnPlot`
- `figures/vln_cldn4_by_mouse.{png,pdf}` — honest n = 6
- `figures/vln_modules_by_restore.{png,pdf}` — AT2 / IFN / MHC-I / TJ AMS
- `figures/vln_modules_cldn4_highlow.{png,pdf}` — Q1 vs Q4
- `figures/dotplot_restore.{png,pdf}` — Seurat `DotPlot`
- `figures/dimplot_pca.{png,pdf}` / `dimplot_umap.{png,pdf}` — `DimPlot`
- `figures/featureplot_cldn4_umap.{png,pdf}` — Cldn4 on UMAP

## How to read this

- n=3 vs 3 mice. Complete Cldn4 / AT2 / Stk11 separation; exact MWU still 0.1.
...
- Sibling Python folder `methods/gse179502_lkb1_cldn4/` is not this run.
  This folder is the Seurat object.
