#!/usr/bin/env python3
"""CosMx He2022: TJ panel genes vs cytotoxic neighbors (immune-cold).

Official figshare clustered object (25976224). Pre-specified panel:
CLDN4, CLDN3, CLDN7, OCLN, F11R, CDH1.

Only genes present in adata.var_names are scored. Off-panel genes are OFF
(not fabricated). Primary cut = Q4 vs Q1 among author-tumor cells; secondary
= detected vs absent. Endpoint = CD8+NK neighbor count ratio at 50/100 µm.

Does not replace the locked CLDN4 ratios 0.36 / 0.52 (different cut/pipeline).
"""
...
H5AD = Path(
    os.environ.get(
        "COSMX_H5AD",
        str(ROOT.parent / "data" / "cosmx_nsclc" / "cosmx_human_nsclc_clustered.h5ad"),
    )
)
...
def section_ratio(adata, gene: str, radius_um: float, mode: str) -> list[dict]:
    ...
    for sample in SAMPLES:
        ...
        say(f"  {gene} {mode} {radius_um}µm {sample}: {mh:.3f}/{ml:.3f}={mh/ml if ml else float('nan'):.3f}")
    return rows
...
def main():
    ...
    for g in TJ_PANEL:
        ...
        inv.append({"gene": g, "on_960_panel": g in panel_960, "in_h5ad": on_h5ad, "scored": on_h5ad})
        if on_h5ad:
            on_genes.append(g)
        else:
            say(f"OFF: {g}")
    ...
    # median NN sanity on one section
    ...
    scored = [
        s
        for s in summaries
        if s["status"] == "SCORED" and s["mode"] == "q4_q1" and s["radius_um"] == 50 and s["n_usable_sections"] > 0
    ]
    scored = sorted(scored, key=lambda r: r["ratio"] if np.isfinite(r["ratio"]) else 999)
    rank_rows = [{**r, "rank_coldest_ratio": i, "is_lead_point_estimate": i == 1} for i, r in enumerate(scored, 1)]
    write_tsv(TAB / "cosmx_rank_50um_q4q1.tsv", rank_rows)

    if scored:
        fig, ax = plt.subplots(figsize=(6.5, 3.6))
        genes = [r["gene"] for r in scored]
        ratios = [r["ratio"] for r in scored]
        colors = ["#b45309" if g == "CLDN4" else "#1f2937" for g in genes]
        ax.barh(range(len(genes)), ratios, color=colors)
        ax.set_yticks(range(len(genes)))
        ax.set_yticklabels(genes)
        ax.axvline(1.0, color="#9ca3af", lw=1)
        ax.set_xlabel("CD8+NK neighbor ratio (Q4/Q1) @ 50 µm")
        ax.set_title("CosMx on-panel TJ genes (CLDN3/7/OCLN/F11R OFF)")
        fig.tight_layout()
        fig.savefig(FIG / "cosmx_tj_ratio_50um.png", dpi=150)
        fig.savefig(FIG / "cosmx_tj_ratio_50um.pdf")
        plt.close(fig)
        ...
        json.dump(
            {
                ...
                "on_genes": on_genes,
                "off_genes": [g for g in TJ_PANEL if g not in on_genes],
                "note": "Locked CLDN4 0.36/0.52 not recomputed; this is a same-cut head-to-head.",
            },
            handle,
            indent=2,
        )
    for r in rank_rows:
        say(f"rank{r['rank_coldest_ratio']} {r['gene']} ratio={r['ratio']:.3f} neg={r['n_sections_neg']}/{r['n_usable_sections']}")
