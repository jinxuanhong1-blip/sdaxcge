...
Parses the official `interactors` column (e.g. CXCL10-CXCR3, IFNG-IFNGR1+IFNGR2,
HLA-A-KIR3DL1). Hyphenated gene symbols (HLA-A) are split only when both sides
match the CellPhoneDB gene list. Complexes use '+' subunits.

This is the pair list used by the lightweight score. It is not CellChat output.
"""
...
def parse_interactors(text: str, genes: set[str]) -> tuple[str, str] | None:
    ...
    for i, ch in enumerate(text):
        ...
        if not left or not right:
            continue
        left_ok = all(p in genes for p in left.split("+"))
        right_ok = all(p in genes for p in right.split("+"))
...
def main() -> None:
    ...
    ap.add_argument("--outdir", type=Path, default=HERE / "resources")
    args = ap.parse_args()
    ...
    # HLA and interferon symbols that appear in interactors.
    valid.update(
        {
            "HLA-A",
            "HLA-B",
    ...
    print(pairs["pathway"].value_counts().to_string())
