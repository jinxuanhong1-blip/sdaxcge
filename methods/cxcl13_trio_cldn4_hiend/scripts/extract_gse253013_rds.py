...
# Malignant / epithelial / T / NK / B / TLS12 panel (additive TLS meta).
def _load_panel() -> list[str]:
    here = Path(__file__).resolve()
    candidates = [
        here.parents[1] / "db" / "gene_panel.txt",
        Path("/workspace/methods/cxcl13_trio_cldn4_hiend/db/gene_panel.txt"),
    ]
    for p in candidates:
        if p.exists():
            genes = [ln.strip() for ln in p.read_text().splitlines() if ln.strip() and not ln.startswith("#")]
            if genes:
                return genes
    return [
        "TACSTD2", "CLDN4", "EPCAM", "KRT8", "KRT18", "KRT19", "KRT7", "CDH1",
        "PTPRC", "CD3D", "CD3E", "CD2", "CD8A", "CD4", "NKG7", "GNLY", "CXCL13",
    ]

GENE_PANEL = _load_panel()
TYPE_NAMES = {
    0: "NIL",
    1: "SYM",
    2: "LIST",
