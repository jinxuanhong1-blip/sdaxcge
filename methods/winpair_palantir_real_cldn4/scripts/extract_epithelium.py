...
ADDITIVE. Epithelium only. GSE207422 and GSE148071 are not added.
GSE131907: author Epithelial cells on tLung / nLung / tL/B / mLN / mBrain
(PE unlabeled epithelium is dropped). nLung AT2 is kept for the Palantir early cell.
GSE205335: author lineage.total == Epithelial cells on non-normal tissues.
Patient is the inferential unit; sample-level cap is only for object size.
"""
...
def extract_gse131907(data: Path, gene_cap: int | None, cap: int) -> "ad.AnnData":
    ...
    adata.obs["patient_key"] = "GSE131907:" + adata.obs["patient_id"].astype(str)
...
def extract_gse205335(data: Path, cap: int) -> "ad.AnnData":
    ...
    adata.obs["patient_key"] = "GSE205335:" + adata.obs["patient"].astype(str)
    adata.layers["counts"] = adata.X.copy()
...
def main() -> None:
    ...
    del a, b
    if "patient_key" not in joint.obs.columns:
        pid = joint.obs["patient_id"].astype(str) if "patient_id" in joint.obs else "NA"
        joint.obs["patient_key"] = joint.obs["dataset"].astype(str) + ":" + pid
    ...
    inv = {
        ...
        "n_patients": int(joint.obs["patient_key"].nunique()) if "patient_key" in joint.obs else None,
