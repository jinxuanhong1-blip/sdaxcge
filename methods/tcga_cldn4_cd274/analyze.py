...
def write_finding(stats: pd.DataFrame, counts: pd.DataFrame) -> None:
    ...
    lines.append("")
    lines.append("**Unadjusted: null in both histologies.** CLDN4 is not associated with CD274 or HLA-A/B/C on the raw HiSeqV2 matrix. LUAD |ρ| ≤ 0.059 (all p ≥ 0.18). LUSC |ρ| ≤ 0.032 (all p ≥ 0.47).")
    lines.append("")
    lines.append(f"**After ImmuneScore: small LUAD residual; LUSC stays null.** LUAD n={luad_n}: CLDN4 vs CD274 {describe('TCGA-LUAD', 'CD274')}; vs HLA-A {describe('TCGA-LUAD', 'HLA-A')}; vs HLA-C {describe('TCGA-LUAD', 'HLA-C')}. CD274 and HLA-A partials survive BH q=0.036 across the 8 primary tests. HLA-C does not (q=0.168). Effect size is |ρ| ≈ 0.11 (~1% of rank variance). LUSC n={lusc_n} remains null on every endpoint (all partial p ≥ 0.31).")
    lines.append("")
    lines.append("The LUAD residual is a **suppressor** pattern, not an immune-cold claim. CLDN4 vs ImmuneScore is weakly negative and non-significant (ρ=−0.065, p=0.14). CD274 and HLA track ImmuneScore strongly (ρ=+0.52 to +0.70). Removing infiltrate unmasks a small positive CLDN4–CD274 / CLDN4–HLA-A association that the unadjusted test does not show. Do not quote the partial without the unadjusted null.")
    lines.append("")
    lines.append("**Do not over-read HLA-B / MHC-I partials.** HLA-B ∈ Immune141. Residualizing ImmuneScore out of HLA-B subtracts a score that already contains HLA-B. HLA-A, HLA-C, and CD274 are the non-circular endpoints.")
    lines.append("")
    lines.append("LUSC dropped **1** HiSeqV2 primary that has no official ESTIMATE row (502 → 501). That is the honest analysis n, not 502.")
    lines.append("")
