...
2. Confirmatory: all-cancers median-abundance rank of CLDN4 among the same claudins.
3. Confirmatory: pairwise ρ(TACSTD2, CLDN4) at RNA and protein.
4. Descriptive: genome-wide abundance rank of TACSTD2 and of CLDN4.
...
- Claudin family: gene symbols matching `^CLDN\d+$` present in that matrix. CLDND1 and similar non-integer symbols are excluded. TACSTD2 is not a claudin and is not inserted into the claudin rank list.
...
Family: all `CLDN\d+` genes tested against TACSTD2 **within one layer × ALL_CANCERS** (RNA and protein are separate families). m_RNA = 24, m_protein = 10. Raw p: two-sided Spearman. Correction: Benjamini–Hochberg. Prespecified q threshold: 0.05. Ties in ρ use minimum rank. Lineage tests are exploratory and are **not** folded into the confirmatory BH family.

Confirmatory RNA family: 18 / 24 claudins with q<0.05.  
Confirmatory protein family: 6 / 10 claudins with q<0.05.

### Results
**Confirmatory, all cancers — coexpression rank (primary B2).**

RNA (DepMap 24Q2, n=1517 lines): TACSTD2 vs CLDN4 ρ = 0.714, p = 7.82e-237, n_pairwise = 1517. CLDN4 rank among claudins by ρ = **2 / 24**. Top claudin = **CLDN7** (ρ = 0.726). CLDN4 is NOT the top TACSTD2-coexpressed claudin at RNA (rank 2 of 24; top is CLDN7).

Protein (Nusinow 2020, n=375 lines): TACSTD2 vs CLDN4 ρ = 0.619, p = 3.47e-22, n_pairwise = 197. CLDN4 rank among claudins by ρ = **1 / 10**. Top claudin = **CLDN4** (ρ = 0.619). CLDN4 is the top TACSTD2-coexpressed claudin at protein in Nusinow CCLE lines.

**Confirmatory — abundance rank among claudins (median across lines).**  
RNA: CLDN4 median-abundance rank = 4 / 24.  
Protein: CLDN4 median-abundance rank = 2 / 10.

**Descriptive — genome-wide abundance rank.**  
RNA mean log2(TPM+1): TACSTD2 rank 8367 / 19193; CLDN4 rank 5906 / 19193.  
Protein median TMT: TACSTD2 rank 12043 / 12196; CLDN4 rank 7587 / 12196.
...
2. 验证性：全癌种 CLDN4 在同一家族中的中位丰度位次。
3. 验证性：TACSTD2–CLDN4 成对 ρ（RNA 与蛋白）。
4. 描述性：TACSTD2 与 CLDN4 的全基因组丰度位次。
...
### 方法与可复现性
- Claudin 家族：该矩阵中匹配 `^CLDN\d+$` 的基因。CLDND1 等非整数符号不计入。TACSTD2 不是 claudin，不进入 claudin 位次表。
...
### Multiple testing / 多重检验
检验族：在**单一层次 × ALL_CANCERS** 内，全部 `CLDN\d+` 对 TACSTD2（RNA 与蛋白分族）。m_RNA = 24，m_protein = 10。原始 p：双侧 Spearman。校正：Benjamini–Hochberg。预设 q 阈值：0.05。ρ 并列时取最小位次。谱系检验为探索性，**不**并入验证性 BH 族。

验证性 RNA 族：18 / 24 个 claudin q<0.05。  
验证性蛋白族：6 / 10 个 claudin q<0.05。

### 结果
**验证性，全癌种 — 共表达位次（B2 主终点）。**

RNA（DepMap 24Q2，n=1517 系）：TACSTD2 vs CLDN4 ρ = 0.714，p = 7.82e-237，成对 n = 1517。CLDN4 按 ρ 在 claudin 中的位次 = **2 / 24**。最高 claudin = **CLDN7**（ρ = 0.726）。CLDN4 is NOT the top TACSTD2-coexpressed claudin at RNA (rank 2 of 24; top is CLDN7)。

蛋白（Nusinow 2020，n=375 系）：TACSTD2 vs CLDN4 ρ = 0.619，p = 3.47e-22，成对 n = 197。CLDN4 按 ρ 在 claudin 中的位次 = **1 / 10**。最高 claudin = **CLDN4**（ρ = 0.619）。CLDN4 is the top TACSTD2-coexpressed claudin at protein in Nusinow CCLE lines。

**验证性 — claudin 家族中位丰度位次。**  
RNA：CLDN4 中位丰度位次 = 4 / 24。  
蛋白：CLDN4 中位丰度位次 = 2 / 10。

**描述性 — 全基因组丰度位次。**  
RNA 平均 log2(TPM+1)：TACSTD2 第 8367 / 19193；CLDN4 第 5906 / 19193。  
蛋白中位 TMT：TACSTD2 第 12043 / 12196；CLDN4 第 7587 / 12196。
